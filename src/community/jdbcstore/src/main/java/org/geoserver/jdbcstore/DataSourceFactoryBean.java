...
// TODO copied from JDBCConfig, should refactor to remove duplicate code
public class DataSourceFactoryBean implements FactoryBean<DataSource>, DisposableBean {
    ...
    private static Context getJNDI(JDBCResourceStoreProperties config) {
    ...
    public DataSourceFactoryBean(JDBCResourceStoreProperties config) {
        this(config, getJNDI(config));
    }

    
    public DataSourceFactoryBean(JDBCResourceStoreProperties config, Context jndiCtx) {
        this.config = config;
    ...
    protected DataSource lookupOrCreateDataSource() throws Exception {
        ...
        try {
            ...
            if(jndi) {
                LOGGER.severe("Error connecting to JDBC Resource Store database. Verify the settings of your JNDI data source and that the database is available.");
            } else {
                LOGGER.severe("Error connecting to JDBC Resource Store database. Verify the settings in jdbcstore/jdbcstore.properties and that the database is available.");
            }
            throw ex;
        }
        ...
        if(name.isPresent()) {
            try {
                ...
                if(LOGGER.isLoggable(Level.INFO)) {
                    LOGGER.log(Level.INFO, "JDBC Resource Store using JNDI DataSource {0}", name.get());
                }
    ...
    /**
     * Create and configure a DataSource based on the JDBCResourceStoreProperties
     * @return
     * @throws Exception
     */
    protected DataSource createDataSource() throws Exception {
        ...
        if(LOGGER.isLoggable(Level.INFO)) {
            LOGGER.log(Level.INFO, "JDBC Resource Store using JDBC DataSource {0}", config.getJdbcUrl());
        }
